...
(function () {
  ...
  loadCmsContent().then(function () {
    ...
    // Il contenuto dinamico (Seminari, Frammenti, Quaderni, Testimonianze)
    // viene caricato in modo asincrono: lo scroll automatico del browser
    // verso un'eventuale ancora nell'URL (es. #quaderni) avviene PRIMA che
    // questo contenuto sia effettivamente presente, quindi punta a una
    // posizione poi resa scorretta dalla crescita delle sezioni sopra.
    // Una volta che tutto e' caricato e il layout e' definitivo, si
    // ripete lo scroll verso la stessa ancora, questa volta corretta.
    // Se il popup di iscrizione e' gia' aperto (flusso ?iscriviti=1, che
    // gestisce lo scroll a modo suo) non si interviene, per non interferire.
    if (window.location.hash && (!modal || modal.hidden)) {
      var targetSection = document.querySelector(window.location.hash);
      if (targetSection) targetSection.scrollIntoView({ behavior: "instant", block: "start" });
    }
  });
